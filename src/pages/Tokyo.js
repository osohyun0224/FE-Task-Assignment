...
export default function Tokyo() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const weatherResponse = await fetchWeather('Tokyo');
      if (weatherResponse) {
      ...
      const forecastResponse = await fetchForecast('Tokyo');
  ...
  return (
    <div className={styles.pageContainer}>
      ...
      <h1 className={styles.title}>Weather Information for Tokyo</h1>
